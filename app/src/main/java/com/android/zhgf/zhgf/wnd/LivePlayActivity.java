...
public class LivePlayActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent in = getIntent();
        mPlayURL = in.getStringExtra("RTMP_URL");
        if (mPlayURL=="" || mPlayURL.indexOf("rtmp://")<0){
            DialogUtil.Toast(getApplication(),"无法获取对方视频地址！");
            finish();
        }else{
            Log.e(TAG, "onCreate: "+mPlayURL);
        }

        dm = getResources().getDisplayMetrics();

        showLog = true;//(Boolean) SharedPreUtil.getBoolean(this, "enablePlayLog");
        enableVideo = true;//(Boolean) SharedPreUtil.getBoolean(this, "enableVideo");
        ...
        sv = (SurfaceView) findViewById(R.id.mLivePlaySurface);
        ...
        capBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (LivePlayer.capturePicture(capFilePath)) {
                    Toast.makeText(LivePlayActivity.this, "截图保存到 " + capFilePath, Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(LivePlayActivity.this, "截图保存失败", Toast.LENGTH_SHORT).show();
                }
            }
        });
         ...
         */
        int bufferTime = Integer.valueOf(2000); // 获取上一个页面设置的bufferTIme，非sdk方法
         ...
         * 这个值关乎延迟的大小。
         */
        int maxBufferTime = Integer.valueOf(5000);// 获取上一个页面设置的maxBufferTIme，非sdk方法
